...
async function consultarCriptos() {
	...
	try {
		...
		const criptomonedas = await obtenerCriptos(resultado.Data)
...
function submitForm(e) {
	...
	// Consultar API con la cotización
	consultarAPI()
}

function mostrarAlerta(msg) {
	const error = document.querySelector('.error')
	// No mostrar la alerta más de una vez
	if (!error) {
		...
		// Quitar mensaje luego de tres segundos
		setTimeout(() => {
			divMensaje.remove()
		}, 3000)
	}
